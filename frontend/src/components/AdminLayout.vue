...
<template>
  <div
    ...
    <aside :class="[
      ...
      <div class="p-6 border-b border-[var(--outline)] flex items-center gap-3">
        <div class="w-8 h-8 bg-[var(--primary)] flex items-center justify-center">
          <span class="text-[var(--on-primary)] font-bold text-xs"
            style="font-family:'JetBrains Mono',monospace;">&gt;_</span>
        </div>
        <span class="font-bold text-lg tracking-tighter text-[var(--on-surface)]">&lt;KEN_BE_CODED&gt;</span>
      </div>

      <!-- Nav -->
      <nav class="flex-grow p-4 space-y-1 overflow-y-auto">
        <router-link v-for="item in navItems" :key="item.label" :to="item.to" @click="sidebarOpen = false" :class="[
      ...
      <div class="p-6 border-t border-[var(--outline)]">
        <div class="flex items-center gap-3">
          <div
            class="w-10 h-10 bg-[var(--primary)] flex items-center justify-center text-[var(--on-primary)] font-bold text-sm"
            style="font-family:'JetBrains Mono',monospace;">AD</div>
          <div>
            <p class="text-[12px] font-semibold tracking-widest uppercase text-[var(--on-surface)]"
              style="font-family:'JetBrains Mono',monospace;">Admin_Root</p>
            <p class="text-[10px] tracking-widest uppercase text-[var(--on-surface-variant)]"
              style="font-family:'JetBrains Mono',monospace;">Session: Active</p>
          </div>
        </div>
        <button @click="logout"
          ...
          style="font-family:'JetBrains Mono',monospace;">[ LOGOUT ]</button>
      </div>
    </aside>
    ...
    <div class="flex-grow lg:ml-64 flex flex-col min-h-screen">
      ...
      <!-- Top Bar -->
      <header
        class="sticky top-0 w-full z-40 flex justify-between items-center px-5 md:px-16 py-4 bg-[var(--background)]/90 backdrop-blur-md border-b border-[var(--outline)]">
        ...
        <div class="flex items-center gap-4">
          <button class="text-[var(--primary-bright)] hover:opacity-70 transition-opacity"><span
              class="material-symbols-outlined">search</span></button>
          <button class="text-[var(--primary-bright)] hover:opacity-70 transition-opacity"><span
              class="material-symbols-outlined">notifications</span></button>
          <button @click="toggleTheme" class="text-[var(--primary-bright)] hover:opacity-70 transition-opacity">
